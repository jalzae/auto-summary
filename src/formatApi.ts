...
export function formatDart(result: any, extension: string = "dart") {
  for (const item of result) {
    ...
    for (const i of item.items) {
      ...
      //cek post or put 
      if (i.method == "POST" || i.method == "PUT") content += `{Map<String,dynamic> data=const {},Map<String,dynamic> headers=const {}}`
      ...
      if (i.method == "POST" || i.method == "PUT") content += `,"body":data,"header":headers`
      content += `};`
